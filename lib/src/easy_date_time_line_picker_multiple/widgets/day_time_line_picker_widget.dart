...
import '../../easy_date_time_line_picker/disable_strategy/strategies.dart';
...
part '../controllers/easy_date_picker_multiple_controller.dart';

class DayTimeLinePickerMultipleWidget extends StatefulWidget {
  ...
  const DayTimeLinePickerMultipleWidget({
    super.key,
  ...
  ///{@macro controller}
  final EasyDatePickerMultipleController? controller;
  ...
  final OnMultipleDateChangeCallBack? onDateChange;
  ...
  /// {@macro header_type}
  final HeaderTypeV2 headerType;

  /// {@macro month_year_picker_options}
  final MonthYearPickerOptions monthYearPickerOptions;

  @override
  State<DayTimeLinePickerMultipleWidget> createState() =>
      _DayTimeLinePickerMultipleWidgetState();
}

class _DayTimeLinePickerMultipleWidgetState
    extends State<DayTimeLinePickerMultipleWidget> {
  late double _itemExtent;
  late int _itemCount;
  EasyDatePickerMultipleController? _internalController;
  EasyDatePickerMultipleController get _controller =>
  ...
  void initState() {
    ...
    if (widget.controller == null) {
      _internalController = EasyDatePickerMultipleController();
    }
  ...
  @override
  void didUpdateWidget(covariant DayTimeLinePickerMultipleWidget oldWidget) {
  ...
  Future<void> _showMonthYearPicker(BuildContext context) async {
    final pickedDate = await showMonthYearPickerFromOptions(
      context: context,
      focusDate: widget.focusedDate,
      currentDate: widget.currentDates.first,
      firstDate: widget.firstDate,
      lastDate: widget.lastDate,
      locale: localeFromLanguageTag(widget.locale),
      disableStrategy: widget.disableStrategy,
      ...
      children: [
        if (widget.headerBuilder != null)
          ...
          switch (widget.headerType) {
            HeaderTypeV2.viewOnly => PickerButtonViewOnly(
                timelinePadding: widget.timelineOptions.padding,
                date: widget.focusedDate,
                local: widget.locale,
              ),
            HeaderTypeV2.picker => PickerButton(
                onPressed: () => _showMonthYearPicker(context),
                timelinePadding: widget.timelineOptions.padding,
                date: widget.focusedDate,
                local: widget.locale,
              ),
            HeaderTypeV2.none => const SizedBox.shrink(),
          },
        SizedBox(
  ...
  void _onDateChanged(DateTime date) {
    final existingIndex = _selectedDates.indexWhere(
    ...
    // Llamamos al callback con la lista actualizada
    widget.onDateChange?.call(List.unmodifiable(_selectedDates));
